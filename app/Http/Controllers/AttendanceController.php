...
########################## this code update attendas hours when you update status from attendance to upsent #############################
#########################################33###########3

    $total_hours_amount1  = Attendance::where('employer_id', $employer_id)->where('value_status', 1)->pluck('total_hours_amount')->first();
    $total_hours_amount2 =   $total_hours_amount1 - $diff_hours;
...
###########################################################
                // when Absent the employee
                    ...
                    if($absent_date_exists){
                        ...
                        $attendance->update([
                            ...
                            'total_hours_amount' => $total_hours_amount2,
               ...
               // calculate total upsent days and equel to report
                    ...
                    // return $total_absent_days;
                     ...
                     $total_hours_amount_apsent  = Attendance::where('employer_id', $employer_id)->whereYear('date', $year)
                     ->whereMonth('date', $month)
                     ->sum('total_hours_amount');

                     $total_attendance = ($total_hours_amount_apsent * $total_hour_price);
                     #########################################################################
                     #########################################################################
                        // write report
                        ...
                        if($report){
                            ...
                            $update_report->update([
                                ...
                                'total_hours_amount'       =>$total_hours_amount_apsent,
                                'total'                 => $total_attendance,  // you might write absent day discount
...
###########################################################
            ...
            $attendances->update([
                ...
                // $total_hours_amount  = Attendance::where('employer_id', $employer_id)->where('value_status', 1)->pluck('total_hours_amount')->first();
                // return $total_hours_amount2;
                 ...
                 // return $total_absent_days;
                    ...
                    //  $discount_all_total_of_days = $discount_total * $total_absent_days;
                     ...
                     #########################################################################
                     ################################### testing ########################################3
                    //  $attendance = Attendance::where('employer_id',$employer_id)->first();
                    $attendance34 = Attendance::where('value_status', 1)->where('employer_id', $employer_id)->exists(); // if he exist 

                     
                     if($attendance !=null ){

                     $diff_hours_over2  = Attendance::where('employer_id', $employer_id)->where('value_status', 1)->pluck('diff_hours_over')->first();
                     $diff_hours_discount3  = Attendance::where('employer_id', $employer_id)->where('value_status', 1)->pluck('diff_hours_discount')->first();
                     }
                    //  return $attendance;
                     ##########################################################################
                     // write report
                    ...
                    //  return $report;
                     if($report && $attendance34){
                         ...
                         $update_report->update([
                             ...
                             'total_attendace_days'     =>$total_attendance_days,
                             'total_hours_amount'       =>$total_hours_amount_apsent,
                             'total_hours_overtime'=>$diff_hours_over2,
                             'total_hours_discount'=>$diff_hours_discount3, 
    ...
    } else
{
    // return $request->all();
    // is he attendance
    $attendance_exists = Attendance::where('absent_date', $employee_date)->where('employer_id', $employer_id)->exists(); // if he exist 
...
//////////////////////
    ...
    public function search(Request $request)
    {
        ...
        //     $attendances  = Attendance ::all();
            ...
            return back()->with('error','No query found');
        }

        return view('backend.Attendance.index',compact('attendances','employers'));
    } 
...
public function Search_attendances(Request $request)
{
 ...
 // في حالة عدم تحديد تاريخ
        ...
        // في حالة تحديد تاريخ استحقاق
            ...
            return back()->with('error','there is no attendance by this date yet');
